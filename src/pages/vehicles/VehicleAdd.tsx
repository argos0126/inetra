...
export default function VehicleAdd() {
  ...
  useEffect(() => {
    const fetchRefs = async () => {
      const [vt, tr, ta] = await Promise.all([
      ...
      if (tr.data) setTransporters(tr.data);
      if (ta.data) setTrackingAssets(ta.data);
    };
    fetchRefs();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isUnique = await checkUniqueVehicleNumber(formData.vehicle_number);
    if (!isUnique) {
    ...
    if (formData.tracking_asset_id) {
      const assetCheck = await checkTrackingAssetNotMappedToActiveVehicle(formData.tracking_asset_id);
    ...
    // Warn about compliance issues but allow save
    if (complianceErrors.length > 0) {
      toast({ 
        title: "Compliance Warning", 
        description: "Vehicle has expired compliance documents. Please update them soon.", 
        variant: "destructive" 
      });
    }
    ...
    try {
      const { error } = await supabase.from("vehicles").insert({
        ...
        tracking_asset_id: formData.tracking_asset_id || null,
        make: formData.make || null, model: formData.model || null, year: formData.year || null,
        is_dedicated: formData.is_dedicated, location_code: formData.location_code || null,
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <div><h1 className="text-3xl font-bold">Add Vehicle</h1><p className="text-muted-foreground">Create a new vehicle record</p></div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-6">
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.transporter_id} onValueChange={(v) => setFormData({ ...formData, transporter_id: v })}>
                    ...
                    <SelectContent>{transporters.map(t => <SelectItem key={t.id} value={t.id}>{t.transporter_name}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label>Tracking Asset</Label>
                  <Select value={formData.tracking_asset_id} onValueChange={(v) => setFormData({ ...formData, tracking_asset_id: v })}>
                    <SelectTrigger><SelectValue placeholder="Select asset" /></SelectTrigger>
                    <SelectContent>{trackingAssets.map(ta => <SelectItem key={ta.id} value={ta.id}>{ta.display_name}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                ...
                <div className="flex items-center space-x-2 pt-6">
                  <Checkbox checked={formData.is_dedicated} onCheckedChange={(c) => setFormData({ ...formData, is_dedicated: !!c })} />
                  <Label>Dedicated Vehicle</Label>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="flex justify-end space-x-4">
              ...
              <Button type="submit" disabled={loading}><Save className="h-4 w-4 mr-2" />{loading ? "Saving..." : "Save Vehicle"}</Button>
            </div>
          </div>
        </form>
      </div>
