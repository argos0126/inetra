...
export default function DriverEdit() {
  ...
  const fetchData = async () => {
    try {
      const [driverRes, trRes] = await Promise.all([
      ...
      if (!driverRes.data) { navigate("/drivers"); return; }
      const d = driverRes.data;
      setFormData({
        name: d.name || "", mobile: d.mobile || "", email: d.email || "", transporter_id: d.transporter_id || "", is_dedicated: d.is_dedicated,
        location_code: d.location_code || "",
        license_number: d.license_number || "", license_issue_date: d.license_issue_date || "", license_expiry_date: d.license_expiry_date || "",
        consent_status: d.consent_status || "not_requested",
        aadhaar_number: d.aadhaar_number || "", aadhaar_verified: d.aadhaar_verified || false,
        pan_number: d.pan_number || "", pan_verified: d.pan_verified || false,
        voter_id: d.voter_id || "", passport_number: d.passport_number || "",
        police_verification_date: d.police_verification_date || "", police_verification_expiry: d.police_verification_expiry || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      // Check for duplicates (excluding current driver) including email
      const [mobileUnique, licenseUnique, aadhaarUnique, panUnique, emailUnique] = await Promise.all([
        checkUniqueDriverMobile(formData.mobile.trim(), id),
        formData.license_number.trim() ? checkUniqueLicenseNumber(formData.license_number.trim(), id) : Promise.resolve(true),
        formData.aadhaar_number.trim() ? checkUniqueDriverAadhaar(formData.aadhaar_number.trim(), id) : Promise.resolve(true),
        formData.pan_number.trim() ? checkUniqueDriverPAN(formData.pan_number.trim(), id) : Promise.resolve(true),
        formData.email?.trim() ? checkUniqueDriverEmail(formData.email.trim(), id) : Promise.resolve(true)
      ]);
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          <Button variant="ghost" size="icon" onClick={() => navigate(`/drivers/${id}`)}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div>
            <h1 className="text-3xl font-bold">Edit Driver</h1>
            <p className="text-muted-foreground">Update driver details</p>
          </div>
        </div>
