...
export default function CustomerAdd() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check unique fields (email, phone, integration code)
    const [emailUnique, phoneUnique, codeUnique] = await Promise.all([
      formData.email ? checkUniqueCustomerEmail(formData.email) : Promise.resolve(true),
      formData.phone ? checkUniquePhone(formData.phone, 'customer') : Promise.resolve(true),
      formData.integration_code ? checkUniqueCustomerCode(formData.integration_code) : Promise.resolve(true)
    ]);
    ...
    try {
      // Insert customer
      const { data: customerData, error } = await supabase.from("customers").insert({
      ...
      if (createLoginAccount && formData.email && customerData) {
        ...
        const { data: userResult, error: userError } = await supabase.functions.invoke("create-entity-user", {
          body: {
            ...
            firstName: formData.display_name.split(" ")[0],
            lastName: formData.display_name.split(" ").slice(1).join(" ") || "",
            roleName: "Shipper User",
        ...
        if (userError) {
          ...
          toast({ 
            title: "Warning", 
            description: "Customer created but login account creation failed: " + userError.message,
            variant: "destructive" 
          });
        } else {
          toast({ 
            title: "Success", 
            description: "Customer created with login account (Shipper User role)" 
          });
          navigate("/customers");
          return;
        }
      ...
      logError(error, "CustomerAdd.handleSubmit");
          ...
          <div className="grid gap-6">
            ...
            <Card className="border-primary/20 bg-primary/5">
              <CardHeader>
                ...
                <CardDescription>
                  A login account with <span className="font-semibold text-primary">Shipper User</span> role will be created for this customer
                </CardDescription>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  A temporary password will be generated and a password reset email will be sent to the customer's email address.
                </p>
              </CardContent>
            </Card>
            ...
            <div className="flex justify-end space-x-4">
              ...
              <Button type="submit" disabled={loading}><Save className="h-4 w-4 mr-2" />{loading ? "Saving..." : "Save Customer"}</Button>
            </div>
          </div>
        </form>
      </div>
