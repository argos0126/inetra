...
export default function ShipmentAdd() {
  ...
  // Validate on form change - only after first submit attempt
  useEffect(() => {
    if (formSubmitted) {
      validateForm();
    }
  ...
  // Auto-populate dimensions when material is selected
  useEffect(() => {
    if (formData.material_id) {
      ...
      if (selectedMaterial) {
        ...
        if (selectedMaterial.length_cm) {
          updates.length_cm = selectedMaterial.length_cm.toString();
          hasUpdates = true;
        }
        if (selectedMaterial.breadth_cm) {
          updates.breadth_cm = selectedMaterial.breadth_cm.toString();
          hasUpdates = true;
        }
        if (selectedMaterial.height_cm) {
          updates.height_cm = selectedMaterial.height_cm.toString();
          hasUpdates = true;
        }
        if (selectedMaterial.weight_kg) {
          updates.weight_kg = selectedMaterial.weight_kg.toString();
          hasUpdates = true;
        }
  ...
  const fetchData = async () => {
    try {
      const [customersRes, locationsRes, materialsRes] = await Promise.all([
        ...
        supabase.from("materials").select("id, name, length_cm, breadth_cm, height_cm, weight_kg, volume_cbm").eq("is_active", true),
      ]);
  ...
  const checkDuplicateCode = async (): Promise<boolean> => {
    if (!formData.shipment_code.trim()) return true;
    
    setDuplicateChecking(true);
    try {
      const isUnique = await checkUniqueShipmentCode(formData.shipment_code.trim());
      if (!isUnique) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Shipment created", description: `${formData.shipment_code} has been added.` });
      navigate("/shipments");
    } catch (error: any) {
      logError(error, "ShipmentAdd");
      toast({ title: "Error creating shipment", description: getDisplayErrorMessage(error), variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const hasErrors = formSubmitted && validationErrors.length > 0;

  return (
    <Layout>
      <div className="space-y-4 sm:space-y-6">
        <PageHeader title="Add New Shipment" description="Create a new shipment entry" />
        ...
        <form onSubmit={handleSubmit}>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {materialAutoFilled && (
                    <p className="text-xs text-muted-foreground">Dimensions auto-filled from material</p>
                  )}
                </div>
              </CardContent>
            </Card>
          ...
          <div className="flex justify-end gap-4 mt-6">
            <Button type="button" variant="outline" onClick={() => navigate("/shipments")}>Cancel</Button>
            <Button type="submit" disabled={loading || duplicateChecking || hasErrors}>
              {loading ? "Creating..." : duplicateChecking ? "Checking..." : "Create Shipment"}
            </Button>
          </div>
        </form>
      </div>
